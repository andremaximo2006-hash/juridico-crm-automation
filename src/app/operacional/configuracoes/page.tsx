...
const DEFAULT_COLUMNS: KanbanColumn[] = [
  { id: "novo", name: "Novo", color: "#9ca3af", order: 1 },
  { id: "triagem", name: "Triagem", color: "#3b82f6", order: 2 },
  { id: "em-andamento", name: "Em Andamento", color: "#f59e0b", order: 3 },
  { id: "concluido", name: "Concluído", color: "#22c55e", order: 4 },
];

export default function OperacionalConfigPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("operacional-kanban-config");
    if (saved) {
      try {
  ...
  function handleSave() {
    localStorage.setItem("operacional-kanban-config", JSON.stringify(columns));
    setUnsaved(false);
    setSaved(true);
  ...
  function handleReset() {
    if (confirm("Deseja resetar as configurações para o padrão?")) {
      ...
      localStorage.removeItem("operacional-kanban-config");
      setUnsaved(false);
    }
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-slate-950">
      <Header title="Configurações de Operacional" subtitle="Personalize o quadro de operações" />

      <main className="flex-1 overflow-auto p-6">
        <div className="max-w-2xl mx-auto">
          {/* Colunas do Kanban */}
          <div className="bg-slate-900 rounded-xl p-6 shadow-xl">
            <h2 className="text-xl font-bold text-white mb-6">Colunas do Quadro</h2>
              ...
              {columns.map((column) => (
                ...
                >
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-400">0 procedimentos</p>
                  </div>
                  <div className="p-4 h-32 flex items-center justify-center text-slate-500">
                    <p className="text-xs">Nenhum procedimento</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-8">
            ...
            {showAdvanced && (
              <div className="mt-4 bg-slate-900 rounded-xl p-6 shadow-xl border border-slate-700">
                <div className="space-y-4">
                  <div>
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" defaultChecked className="w-4 h-4 rounded" />
                      <span className="text-sm text-slate-300">Mostrar número de procedimentos nas colunas</span>
                    </label>
                  </div>
                  <div>
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" defaultChecked className="w-4 h-4 rounded" />
                      <span className="text-sm text-slate-300">Permitir drag-and-drop entre colunas</span>
                    </label>
                  </div>
                  <div>
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" className="w-4 h-4 rounded" />
                      <span className="text-sm text-slate-300">Ocultar procedimentos sem observações</span>
                    </label>
                  </div>
                </div>
              </div>
