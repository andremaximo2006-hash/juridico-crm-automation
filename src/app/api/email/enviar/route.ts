import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { getSession } from "@/lib/auth";

export async function POST(request: NextRequest) {
  try {
    const session = await getSession();
    if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

    const body = await request.json();
    const { campaignId } = body;

    if (!campaignId) {
      return NextResponse.json({ error: "Missing campaignId" }, { status: 400 });
    }

    // Buscar campanha
    const campanha = await prisma.emailCampanha.findUnique({
      where: { id: campaignId },
      include: { template: true }
    });

    if (!campanha) {
      return NextResponse.json({ error: "Campaign not found" }, { status: 404 });
    }

    // Atualizar status para "enviando"
    await prisma.emailCampanha.update({
      where: { id: campaignId },
      data: { status: "enviando" }
    });

    // Criar mensagens para cada destinatário
    const mensagens = await Promise.all(
      campanha.destinatarios.map((email: string) =>
        prisma.emailMensagem.create({
          data: {
            campaignId,
            templateId: campanha.templateId,
            paraEmail: email,
            status: "enviando"
          }
        })
      )
    );

    // TODO: Integrar com SendGrid ou SMTP para enviar de verdade
    // Por enquanto apenas simulamos o envio

    // Atualizar status para "enviado"
    await prisma.emailCampanha.update({
      where: { id: campaignId },
      data: {
        status: "enviado",
        enviadoEm: new Date(),
        enviados: mensagens.length
      }
    });

    // Atualizar mensagens também
    await prisma.emailMensagem.updateMany({
      where: { campaignId },
      data: { status: "enviado", enviadoEm: new Date() }
    });

    return NextResponse.json({
      success: true,
      campaignId,
      totalEnviados: mensagens.length,
      mensagens: mensagens.length
    });
  } catch (error) {
    return NextResponse.json({ error: String(error) }, { status: 500 });
  }
}
